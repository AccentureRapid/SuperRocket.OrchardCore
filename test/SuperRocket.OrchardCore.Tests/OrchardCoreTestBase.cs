...
using SuperRocket.OrchardCore.Migrations.SeedData;
...
namespace SuperRocket.OrchardCore.Tests
{
    public abstract class OrchardCoreTestBase : AbpIntegratedTestBase<OrchardCoreTestModule>
    {
        ...
        protected void UsingDbContext(Action<OrchardCoreDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OrchardCoreDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OrchardCoreDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OrchardCoreDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OrchardCoreDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrchardCoreDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OrchardCoreDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrchardCoreDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OrchardCoreDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OrchardCoreDbContext, Task<T>> func)
        {
            T result;
